...
- Project Standards is current at v5.13.0 with ADR 1.3, Agent Handoff 1.7, and CLI Documentation 1.5.
- The remaining packages are Markdown Frontmatter 1.7, Markdown Tooling 1.11, Project Specification 1.5, and Python Tooling 1.10.
- The V5 control plane preserves dual Claude/Codex automatic handoff and consumer-owned project knowledge.
- Automatic handoff still conflicts with the active `uv-strict-python` shim on v5.13.0 ([project-standards#80][ps-80]).
- Repository validation passes, but the managed hook exits before injection; platform Python emits one context envelope.
