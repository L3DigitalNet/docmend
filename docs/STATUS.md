...
- Plan D was implemented as eight green commits (`39784b0..f906e9e`) after three audit rounds approved the implementation plan, then fast-forward merged into `dev` and pushed to `origin/dev`. `verify --plan` now certifies repeatable attempt lineage and exactly-once outcomes; restore evidence is lifecycle-only and never requires an apply report.
