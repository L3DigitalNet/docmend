...
| 0.41 | `2026-07-17` | owner + `coding-agent` | **DMR-08 Task 11 reference-equivalence correction** — keeps every public reference field exact except the existing order-independent mount-flag set and permits `ram_bytes` to differ by at most one current Linux base page. Linux `MemTotal` reports usable RAM after kernel reservations rather than immutable installed capacity; the binding file-size matrix exposed a one-page delta on the otherwise-identical accepted environment. The separate observed-RAM minimum remains 16 GiB, wider RAM deltas remain reference mismatches, and the accepted reference document, pilot evidence, and threshold hashes remain unchanged. Owner approved this correction on 2026-07-17 before binding evidence publication. |
| 0.40 | `2026-07-13` | owner + `coding-agent` | **DMR-08 Task 9 pilot and revision-two threshold settlement** — accepts the external uninstrumented 100,000-file installed-wheel pilot for candidate `14f3118e4f57c992b9d5088b9cb4f35fb3658686`, retains the 10,000-file run as diagnostic fit support, and freezes threshold schema 2.0 at a 25,902,581,760-byte absolute 1M peak-RSS ceiling, 25,804-byte/file incremental-slope ceiling, and exact 20% linearity tolerance. The immutable 10k/100k evidence hashes are `sha256:635fee6013364eab9d47d07eab37d99fdb522a9d3ee9dcac05c7939d7648ad00` and `sha256:32f71b4e86549997787c3a63d69f7bbede0d8066c64c3f241d813285e993f0d0`; both bind reference hash `sha256:458b51662c05290784130cf02be812919063121e0ecb33086f5a6373a85f1d55`. Settles OQ-038 through OQ-041, including the exact child-swap transition rule proven by the pilot. NFR-001 remains Partial until the scheduled 100k, file-size, and installed-wheel 1M evidence pass. |
| 0.39 | `2026-07-13` | owner + `coding-agent` | **DMR-08 Task 9 Btrfs capacity contract amendment** — advances scale-evidence to 3.0 so finite statvfs inode pools remain numeric while positively identified Btrfs filesystems with the exact all-zero inode triplet use explicit `dynamic-metadata` mode and null inode availability. Byte capacity remains binding; dynamic mode does not prove Btrfs metadata headroom, which is instead proven by successful corpus materialization and stage/artifact writes. Materialization or harness/artifact-only ENOSPC remains incomplete; a trustworthy nonzero stage exit remains failed under the existing status precedence. Non-Btrfs or unclassifiable zero-inode telemetry fails closed. Threshold schema remains 2.0. |
...
The library is a directory tree of more than 100,000 `.txt` and HTML files accumulated over decades, exhibiting every condition listed in §1: mixed encodings and newline styles, broken formatting, garbage text, corruption, non-descriptive names, and undetected near-duplicates. There is no inventory, no metadata, and no naming convention. Existing duplicate-detection tools fail because of noise and drift in the text. The released v1.0.2 pipeline implements scan, plan, apply, restore, resume, and verify; the post-v1 safety-core Plans A-D add write-once backup ownership, guarded artifact destinations, manifest/report 2.0 lineage and recovery, descriptor-bound commit authority, and plan-aware exactly-once verification. The 2026-07-10 review also found that the historical NFR-001 evidence was invalid for the intended release contract: the test stopped at 100,000 files, measured traced Python allocations inside one process, omitted installed-CLI and `verify --plan` paths, and permitted linear memory growth despite prose requiring corpus-size independence; meanwhile `parallel.*` was parsed but had no runtime consumer. Revision 0.30 therefore started DMR-08 change control: v2.0.0 targets sequential qualification through 1,000,000 files with bounded-linear whole-run metadata, plan schema 2.0, and pilot-derived external-RSS thresholds (OQ-037, `adr-0022-sequential-million-file-scale-contract`). The historical test is replaced on `dev` by an always-on 1,000-file source-tree full-pipeline guard; revisions 0.42 and 0.44 accept the installed-wheel pilot thresholds, file-size matrix, and binding one-million-file release evidence. DMR-08 is closed. The staged real-library write rollout remains paused until DMR-09 and the remaining v2.0.0 release gates close.
...
| --- | --- | --- | --- | --- |
| NFR-001 | Scalability | docmend shall qualify the installed `scan -> plan -> apply --write -> verify --plan` workflow at 1,000,000 files on one accepted Linux/POSIX reference environment. Whole-run artifact metadata may grow linearly with file count; per-file body content shall not accumulate. v2.0.0 executes sequentially. Revision two freezes the executable limits at 25,902,581,760 bytes peak RSS, 25,804 bytes/file incremental slope, and 20% linearity. | The 1,000-file source-tree guard passes in the default gate; accepted installed-wheel 100,000- and 1,000,000-file evidence validates exact corpus conservation and expected findings, separate-process external peak RSS, the revision-two absolute/slope/linearity thresholds, reference-environment identity, and completion of the 1M workflow within 12 hours (OQ-037, OQ-038). | Must |
...
Scale evidence is versioned JSON under `docs/scale-evidence/`, separate from the product `ArtifactKind` registry. An accepted evidence document binds the candidate commit and exact wheel hash; package, build-frontend/backend, Python, kernel, and schema versions; reference-environment hash; corpus count and bytes; per-stage elapsed time, throughput, external peak RSS, structured/stdout/stderr and durable-artifact sizes; monotonic workflow runtime; reconciled outcome totals; and threshold results. Scale-evidence 3.0 represents an ordered attempted stage prefix, separates trustworthy child measurement from durable-artifact validation, permits nullable preflight/exit/measurement fields only where proof is unavailable, and uses a finite outcome reason so incomplete and failed evidence preserves observed truth rather than inventing full-corpus totals. Its per-filesystem capacity record distinguishes `finite-statvfs`, where `available_inodes` is a non-negative integer and the inode comparison contributes to the verdict, from `dynamic-metadata`, where `available_inodes` is null because positively identified Btrfs reported the exact all-zero statvfs inode triplet. Dynamic mode keeps the byte comparison binding but does not claim that statvfs proves Btrfs metadata headroom: successful corpus materialization and all stage/artifact writes are the binding runtime proof. ENOSPC before a trustworthy stage result is incomplete; a trustworthy nonzero stage exit is failed under the existing `stage-exit` precedence. A non-Btrfs, unknown, unreadable, malformed, or ambiguous zero-inode classification fails closed before materialization; all classifications within one preflight derive from one mountinfo snapshot (OQ-039). Passing and otherwise-correct complete diagnostic evidence enforce full phase conservation; failed evidence may preserve the discrepancy it reports. Public evidence rejects hostnames, usernames, absolute paths, serial/device identifiers, document content, and unknown or value-bearing mount options. Supporting pilot points live under `docs/scale-evidence/supporting/`; only complete reviewed baselines live under `docs/scale-evidence/accepted/`, and acceptance never overwrites an existing file. `docs/scale-evidence/thresholds.json` is the executable revision-two source for the frozen 1M peak, per-file slope, and linearity limits; threshold schema 2.0 binds both pilot point hashes, their immutable stage-aligned RSS context, the reference-environment hash, and the fitting method (OQ-038, OQ-041).

Revision two binds `docs/scale-evidence/thresholds.json` to candidate `14f3118e4f57c992b9d5088b9cb4f35fb3658686`. Its 10,000-file diagnostic point (`supporting/14f3118e4f57c992b9d5088b9cb4f35fb3658686-pilot-10000.json`, `sha256:635fee6013364eab9d47d07eab37d99fdb522a9d3ee9dcac05c7939d7648ad00`) records scan/plan/apply/verify peaks of 111,808,512 / 139,542,528 / 219,996,160 / 285,429,760 bytes. Its accepted 100,000-file pilot (`accepted/14f3118e4f57c992b9d5088b9cb4f35fb3658686-pilot-100000.json`, `sha256:32f71b4e86549997787c3a63d69f7bbede0d8066c64c3f241d813285e993f0d0`) records 393,216,000 / 706,932,736 / 1,528,246,272 / 2,143,305,728 bytes, exact 100,000 = 87,500 actions + 10,000 clean no-ops + 2,500 classified skips/findings conservation, zero failures, and zero child swap in every stage. Both points bind `docs/scale-evidence/reference-environment.json` at `sha256:458b51662c05290784130cf02be812919063121e0ecb33086f5a6373a85f1d55`. Exact per-stage linear projection plus 25% upward-rounded headroom yields the executable 25,902,581,760-byte absolute peak and 25,804-byte/file slope limits; linearity remains exactly 0.20 (OQ-038).
...
| Binding environment | Linux/POSIX; at least 16 GiB RAM; local disk-backed ext4, XFS, or btrfs SSD; sufficient free bytes and, where statvfs exposes a finite pool, inodes; no material swap activity. | One accepted public-safe reference class for v2.0.0 qualification. | tmpfs, overlayfs, and network filesystems are diagnostic-only; capacity preflight precedes corpus materialization. Btrfs dynamic inode allocation is runtime-proven by successful writes, not predicted from its all-zero statvfs inode triplet (OQ-039). |
| Memory | Whole-run metadata is bounded-linear; revision-two limits are 25,902,581,760 bytes peak RSS at 1M, 25,804 bytes/file incremental slope, and a 20% linearity band. | The executable limits remain fixed unless a later approved specification revision changes them; a miss never weakens them automatically. | Binding runs measure each stage in a fresh subprocess. Threshold 2.0 applies the frozen limits to current per-stage peak/slope and maximum relative deviation from the immutable 10k/100k pilot projection (OQ-038/OQ-041). |
...
| File size | `limits.max_file_size_mib` is qualified at 100 MiB by the accepted Task 11 file-size matrix. | Independently qualified from file count. | Every configured-maximum-derived UTF-8/Windows-1252 case passes its watchdog and remains below 2 GiB peak RSS; both 100 MiB encodings also pass with tool backups. |
...
Binding scale stages run in fresh subprocesses without `PYTHONTRACEMALLOC`, `-X tracemalloc`, or equivalent allocation tracing. The harness fixes the `uv` frontend and `uv_build` backend, archives an exact clean 40-hex `HEAD` into a new external owner-only workspace, builds/hashes one wheel through the declared backend, installs the hash-locked runtime closure plus that wheel into a fresh venv, verifies metadata/import origins, and invokes only that wheel for the 100k/1M tiers under OQ-041's quiescent same-UID runtime assumption. From source inspection through acceptance, no concurrent process under the invoking UID may mutate the candidate repository, bound interpreter, complete qualification workspace (including installed `site-packages`), or publication destinations. The harness snapshots and reconciles the console script, interpreter binding and resolved target, archived measurement wrapper, held workspace, and clean source at their boundaries; same-UID in-call swap-and-restore, open-inode mutation, ptrace, and process injection are outside the qualification threat model. It validates every artifact passed between stages and records public-safe evidence without hostnames, usernames, absolute paths, device identifiers, or document content. Python allocation peaks are diagnostic-only and never share a run with binding RSS measurement. Child swap sampling retries an unreadable first pre-exec snapshot without publishing a value. After a valid sample, exactly one `State` line with zero `VmSwap` lines is treated only as a recoverable exec/exit memory-map transition: a later valid sample resumes measurement, and a following exact terminal `X`/`Z` state preserves the previous peak. An unresolved transition at poll-only exit, or any other missing, duplicate, malformed, or unreadable post-sample telemetry, publishes null and makes binding evidence incomplete; it never becomes a false zero. Before corpus materialization, deterministic per-filesystem byte/inode accounting includes the exact corpus and largest rendered file plus OQ-041's accepted inventory/plan/report/manifest/verify/log/supervisor/staging coefficients and 64 non-corpus inodes, grouped by followed filesystem with one exact 25% margin. Each private request, result, stderr, and non-verify stdout file retains a 2 MiB ceiling. Verify stdout instead receives `max(2 MiB, 128 bytes × recipe-derived expected findings)`; capacity preflight rounds that derived file size independently at the supervisor filesystem's fragment size. Finite statvfs inode pools receive the numeric inode comparison; only a positively identified Btrfs mount with an exact all-zero inode triplet may use the byte-only `dynamic-metadata` preflight, after which successful materialization and stage/artifact writes prove runtime capacity. Any unclassifiable zero triplet or coefficient overrun makes evidence incomplete. ENOSPC before a trustworthy stage result is also incomplete; a trustworthy nonzero stage exit is failed under the existing precedence. Build/install allocation is already reflected because preflight follows setup (OQ-039, OQ-040, OQ-041).
...
| FR-003 | Stale-hash apply test — `tests/test_apply.py::test_stale_hash__skipped_batch_continues`, `tests/test_cli_apply.py::TestApplyWrites::test_apply_findings__exit_1`; duplicate-run and stale-plan shapes at MS-4 — `tests/test_idempotency.py`; commit-boundary source identity and action-time race coverage — `tests/test_apply.py::TestCommitBoundarySourceBinding`, `tests/test_apply.py::TestCommitBoundaryRaces`. | Complete (MS-3; MS-4 duplicate-run; safety-core Plan C commit boundary) |
| FR-004 | Dry-run default and no-write assertions — `tests/test_cli_apply.py::TestApplyDryRunDefault::test_apply_default__dry_run_writes_nothing`, `tests/test_apply.py::test_dry_run_default__writes_nothing_reports_would_apply`. | Complete (MS-3) |
| FR-005 | Safety-gate refusal tests per missing strategy — `tests/test_gate.py` (refusal-per-missing-strategy set), `tests/test_cli_apply.py::TestApplyGate::test_apply_write_without_strategy__gate_refuses_exit_3`; sealed capability construction and write-entrypoint separation — `tests/unit/writer/test_commit.py::TestWriteSafetyContext`, `tests/test_apply.py::TestEntrypointSplit`. | Complete (MS-3 v1 gate; safety-core Plan C sealed `WriteSafetyContext` engine boundary) |
...
| NFR-004 | Config-default audit test (no mutation out of the box) — `tests/test_cli_apply.py::TestApplyDryRunDefault::test_apply_default__dry_run_writes_nothing`. | Complete (MS-3) |
...
| IR-005 | Global-flag tests incl. `--version` and `--verbose`/`--quiet` exclusivity — `tests/test_cli.py` (flag surface, exclusivity exit 2, version output); level mapping in `tests/test_observability.py`; append `tests/test_cli_apply.py::TestApplyFlagConflicts::test_apply_write_and_dry_run__mutually_exclusive`. | Complete (MS-3: `--dry-run` now has effect) |
...
| OQ-038 | What exact nested public records, artifact-key vocabularies, evidence-reference hashing rule, and threshold rounding semantics complete the approved DMR-08 qualification contract? | Strict aggregate-only records with finite artifact keys; mutually exclusive external-RSS/allocation-diagnostic fields; identifier-free preflight and conservation/finding totals; safe relative evidence identities hashed over exact bytes; immutable stage-aligned 10k/100k RSS; exact non-negative per-stage slopes and 1M projection; 25% projected-peak/slope headroom rounded upward; exact 20% linearity; scheduled two-point and release three-point evaluation rounded only upward; exact installed-wheel provenance and four artifact-validated stages for passing evidence; separate environment-comparison verdict. Revision two freezes 25,902,581,760 bytes peak RSS and 25,804 bytes/file slope. | No | owner | DMR-08 pilot and threshold revision | Resolved in revision 0.40; settled companion record 038 |
| OQ-039 | What exact capacity-margin arithmetic, reference-class equivalence, mount-option projection, dynamic-Btrfs inode representation, and unavailable swap behavior govern binding DMR-08 resource preflight? | Per-file allocation rounding before per-`st_dev` aggregation; exact 25% upward-rounded byte/inode margin per filesystem; scale-evidence 3.0 finite numeric inode pools or null `dynamic-metadata` only for positively identified Btrfs with the exact all-zero statvfs inode triplet; binding byte comparison plus runtime write proof for dynamic mode; fail-closed unclassified zero telemetry; one mountinfo snapshot; exact public reference fields except order-independent mount flags and `ram_bytes` deltas of at most one current Linux base page; wider RAM deltas fail reference comparison, while the observed value independently must satisfy the 16-GiB minimum; Linux/local-SSD/ext4-XFS-btrfs eligibility; finite value-free field-6 flags only, with field-11 options private and non-classifying; unavailable binding child-swap telemetry is incomplete while global swap deltas remain diagnostic. | No | owner | DMR-08 pilot | Resolved in revision 0.40; RAM equivalence amended in revision 0.41; settled companion record 039 |
| OQ-040 | What exact deterministic corpus summary/materialization semantics and private stage request/result/process/file contract complete DMR-08 Task 5? | Strict 1..1,000,000 streamed 40-bucket recipe mix (35 actions / 4 no-ops / 1 skip per full 40); fresh fixed-seed render passes; per-file allocation rounding and complete directory/inode accounting; absent-root owner-only no-follow materialization; strict private stage JSON; tracing-free allowlisted child environment; one child/one explicit wait; 50-ms child-swap sampling; Linux `ru_maxrss * 1024`; and trustworthy-result wrapper exits. An exact one-State/zero-`VmSwap` exec/exit transition may recover on a later valid sample or preserve the prior peak when followed by exact terminal X/Z; unresolved or otherwise invalid post-sample telemetry is null/incomplete, never zero. | No | owner | DMR-08 pilot | Resolved in revision 0.40; settled companion record 040 |
| OQ-041 | What exact partial-evidence, threshold-evaluation, exact-HEAD build, workspace, reference-observation, provisional-capacity, runtime, acceptance, and process-exit contract completes Task 6? | Scale-evidence 3.0 and threshold 2.0; phase-truthful totals and measurement/artifact state; finite status precedence; immutable pilot context and 1M projection; fixed `uv`/`uv_build`; archived clean HEAD plus hash-locked installed wheel under the quiescent same-UID repository/interpreter/workspace/destination assumption; owner-only external workspaces; derived SSD proof and warm cache; accepted conservative capacity coefficients plus 25%, including count-derived verify stdout at `max(2 MiB, 128 bytes × expected findings)`; Btrfs dynamic inode mode; monotonic 43,200-second release verdict; evidence-first no-clobber acceptance; exit 2 before evidence, otherwise 0 only for passing/explicit diagnostic and 1 for failed/incomplete/nonbinding binding requests. | No | owner | DMR-08 orchestrator and pilot | Resolved in revision 0.40; verify-stdout coefficient amended in revision 0.43; settled companion record 041 |
...
- [Linux kernel `/proc` documentation](https://docs.kernel.org/filesystems/proc.html) — defines `MemTotal` as usable RAM after reserved memory and kernel code, which motivates the bounded one-base-page comparison in OQ-039.
