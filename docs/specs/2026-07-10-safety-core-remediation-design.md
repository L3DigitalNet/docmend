...
Approved design for remediating the rollout-blocking safety findings DMR-01 through DMR-07 from the 2026-07-10 comprehensive review synthesis. This document is the design-stage artifact; the binding change lands as a SPEC-VHHB revision plus ADR amendments and new ADRs before implementation (see [Change-control follow-through](#change-control-follow-through)).

Revision note: revised to resolve findings F1–F8 of the design review (artifact-guard default carve-out, descriptor/pathname identity binding on both source and target, the complete manifest 2.0 wire model, backup-store trust boundary, verify input contract, ADR dispositions, and the read/write context split). Revised again per review round 2: the adjudication table now enumerates every crash-after-step state — including the lossless both-names intermediates of the link-then-unlink primitives — and verify's coverage reduction makes the ManifestChain the mutation authority with `already-applied` as a nonterminal confirmation. Revised again per round 3: the object identities the adjudication table consumes are now persisted in intent records, and apply reports carry durable attempt lineage with lifecycle legality stated at both the per-set and cross-set scopes. Revised again per round 4: `expected_published_identity` moves into the intent by staging the output **before** the intent append (so post-publish adjudication has its identity even though the terminal never landed), identity comparison is exact `(st_dev, st_ino)` with device changes refusing rather than substituting, and attempt lineage becomes a discriminated `prior_attempt` reference (report hash or closed-manifest hash) so manifest-with-missing-report attempts stay linkable and missing reports stay findings. Revised again per round 5: the `prior_attempt` edge is persisted redundantly in the apply manifest header **and** the apply report, so the lineage survives whichever artifact an interruption erases (the report-only → manifest-with-missing-report sequence stays connected). Revised again per the Plan A implementation-plan review (F3/F6, spec rev 0.27): guard containment is two-candidate — the lexical directory entry publication would replace plus the fully resolved referent — and the `.docmend/` carve-out is licensed per destination against the effective excludes; artifact file modes stay umask-derived, with permission policy still deferred to sub-project 4. Revised again per the Plan B implementation-plan review round 3 (CR-NEW-002): the per-set lifecycle rule is clarified to provisional standalone-terminal parsing plus mandatory chain-scope closure — resolving this document's internal tension between the old "applied requires a preceding intent within one set" wording and its own adjudication table, whose completed/finish-remaining rows append terminals into later sets; adr-0019's binding wording was already the clarified form. Revised again per the Plan C implementation-plan review round 2 (CR-NEW-002): the lifecycle-legality sentence is disambiguated — the standalone-terminal closure requirement binds adoption (`applied`) terminals only; this document's own "`failed` with no intent anywhere asserts no mutation occurred" clause was already binding, but the chain validator had implemented the ambiguous reading and rejected every manifest containing a pre-mutation failure (fixed with regression tests in the same commit). Revised again on 2026-07-11 per Plan C review CR-006: the manifest header records the run's effective exclude patterns so restore can evaluate the artifact-destination carve-out against apply-time policy that per-invocation replacement flags make unreconstructable later.
...
- `prior_manifest_sha256` is the mutation-ledger subchain link (F4): null for the first apply manifest that mutates; for a resume, the sha256 of the newest earlier manifest in the attempt lineage; for a restore, the sha256 of the newest manifest in the chain it undoes; for a restore re-run, the sha256 of the interrupted restore manifest. Manifest files are closed before any successor hashes them, so the hash is stable (a torn tail stays torn).
- `prior_attempt` is the unified attempt-lineage edge (F6 round 5), persisted **redundantly in the apply manifest header and the apply report** so the edge survives whichever artifact an interruption erases: null for the first attempt, otherwise the predecessor's `run_id` plus either its report sha256 or — when that report was never published — its closed-manifest sha256. The value is computed before mutation from the validated predecessor inputs; when an attempt produces both artifacts, both must carry the identical reference and `run_id`. A report-only attempt persists the edge in its report; a manifest-with-missing-report attempt persists it in its manifest header; a normal attempt persists it in both. The `prior_manifest_sha256` subchain is derived from and validated against this attempt chain.
- `effective_excludes` records the patterns that governed the apply run. Restore evaluates the `.docmend/` artifact-destination carve-out against this durable evidence because per-invocation replacement flags make the effective set impossible to reconstruct later (Plan C review CR-006).
...
- lifecycle legality per action, at two explicit scopes (F6 round 3; clarified per Plan B review round 3, CR-NEW-002): **within one ManifestSet**, at most one terminal record; duplicate applied records are illegal; when an intent and a terminal for one action share a set, their immutable fields must match; a terminal with NO same-set intent parses provisionally — it is legal only when chain scope proves it closes exactly one earlier set's dangling intent for the same action with full immutable-field agreement (the adjudication terminals this document's own table appends), and an unclosed standalone **adoption (`applied`) terminal** is illegal; `failed` with no intent anywhere in the chain asserts no mutation occurred — the producer's pre-mutation failure shape closes nothing and needs no closure evidence (disambiguated per Plan C review round 2, CR-NEW-002: the validator had read "standalone terminal" as covering faileds, wholesale-rejecting any manifest containing an ordinary pre-mutation failure). This provisional-set/strict-chain split is what makes the adjudication table's "append terminal" rows and the worked example's M4 representable; adr-0019's binding wording (per-set terminal uniqueness plus the cross-set transition table) already states exactly this. **Across the chain**, the transition table governs: `failed` (any attempt) → `intent` → `applied` in a later set is legal (a retry); `applied` followed by any contradictory apply-kind terminal for the same action is illegal; restore transitions occur only through `undoes` records; after a `restore`-kind set, further apply-kind sets in the same chain are illegal — re-application requires a fresh plan;
...
- The canonical tool artifact root — `.docmend/` in the invoking directory (OQ-034, §18.2) — remains a legal destination even when it lies inside the corpus root, licensed **per destination** (rev 0.27): each in-corpus candidate must lie under that root **and** have its own corpus-relative path still matched by the effective exclude patterns (the default `**/.docmend/**` exclude keeps its contents out of discovery); a gitignore negation re-including one destination withdraws exactly that destination's license, and an operator-replaced exclude set withdraws the root wholesale. The default workflows (`scan .`, `plan .`, the single-file journey) must keep working without setup (NFR-006), but never by writing into scannable corpus space.
- Every other destination is refused when **either** the lexical directory entry publication would replace (resolved parent plus final name, final component not followed) **or** its fully resolved referent lies inside the corpus root (rev 0.27: `os.replace` swaps directory entries, so resolved-only containment let an in-corpus symlink pointing outward have its corpus-owned entry silently replaced), when it aliases an input artifact of the same invocation, or when it is a non-regular file. Refusal is a safety refusal, exit 3 (ADR-0012), before the pipeline runs — a refused artifact write must not follow a completed scan.
...
Verify consumes Plan, **Report(s)**, ManifestChain, and BackupStore (F6) — matching ADR-0012's input contract (flags or sidecar discovery). **Attempt lineage (F6 rounds 3–5):** one lineage spans all attempt shapes by pointing at whichever durable predecessor evidence exists, and the edge itself is stored redundantly. The discriminated `prior_attempt` reference — null for the first attempt, otherwise the predecessor's `run_id` plus **either** its report sha256 **or**, when that report was never published (the crash-after-manifest-close window), its closed manifest sha256 — is persisted in **both** the apply manifest header and the apply report (which also gains `manifest_sha256`, the sha256 of the attempt's closed manifest, null when the attempt mutated nothing). Whichever artifact an interruption erases, the survivor carries the edge: a report-only predecessor followed by a manifest-with-missing-report attempt stays connected through that manifest's header. Attempt order is this lineage — never caller order or wall-clock — and the missing report **remains a verify finding rather than a structural impossibility**. Manifest↔report cardinality is at most one report per manifest run — absence is the finding, duplication is a contradiction; when both artifacts of one attempt exist, their `prior_attempt` and `run_id` must be identical. A manifest's `prior_manifest_sha256` must reference the newest manifest of an earlier attempt in the same lineage, so the subchain and the attempt chain cannot disagree. CLI discovery: `--resume-run-id` resolves both default sidecars for a predecessor attempt; a repeatable `--prior-report` input names relocated or report-only predecessor reports; a manifest-only predecessor needs no report input — its manifest hash carries the link. Coverage binding: the report's `plan_ref` hash, the manifest headers' `plan_sha256`, and the plan artifact hash must agree. Every confirmed false-clean path becomes a finding:
...
- a verify false-clean matrix asserting each table row yields a finding, plus `verify --plan` full-partition accounting across single-run, resume-chain, dry-run-only, and missing-report cases — including clean single-resume and double-resume chains proving `already-applied` retains the `applied` state, shuffled input order (chain links alone must determine attempt order), a first-action abort with no manifest, an all-skip report-only retry, a failed-manifest-then-successful-retry chain, a manifest-with-missing-report attempt followed by a successful resume, a relocated prior report supplied via `--prior-report`, and the composed chain R1 report-only → R2 manifest-with-missing-report → R3 success with all artifacts passed shuffled, proving one deterministic attempt order with R2's missing report surfacing as a finding, not a broken chain (F6).
