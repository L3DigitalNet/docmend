...
**Workflows** (`.github/workflows/`): `lint-markdown.yml` and `format.yml` are **standard-owned** (project-standards `@v5` control plane via `.standards/`, conventions #8 — do not hand-edit); `check.yml` and `validate-specs.yml` are **consumer-owned** (customized and preserved through the V5 migration via `workflow_ownership = "consumer-owned"`, so they are outside reconciliation); `traceability.yml`, `dependency-review.yml`, and `scale-qualification.yml` are repo-owned additive workflows. `dependency-review.yml` is PR-only and enforces a distribution-tightened license allowlist (no copyleft pre-approved — spec §16/§8.6). `scale-qualification.yml` runs the installed-wheel 100,000-file lane manually or at 06:00 UTC every Monday as a non-binding diagnostic on `ubuntu-latest`; it loads the committed reference and thresholds, never accepts or overwrites binding evidence, and retains the diagnostic evidence plus stage stdout/stderr for 7 days. The reviewed binding one-million-file qualification remains a local clean-HEAD acceptance workflow, not a hosted deployment; its accepted evidence is committed under `docs/scale-evidence/accepted/`.

**Dependabot** (`.github/dependabot.yml`): weekly pip + github-actions, 7-day cooldown, grouped.

**Release: live (`release.yml`, adr-0017, wired at MS-5 as planned).** Signed `vX.Y.Z` tag on `main` → `uv build` (sdist + wheel) → wheel smoke-test → GitHub Release with artifacts attached. No PyPI publishing. Releases: `v1.0.0` (2026-07-07, first), `v1.0.1` (2026-07-07, issue #15 fix), `v1.0.2` (2026-07-07, alignment safety hardening), `v2.0.0` (2026-07-18, safety-core and bounded-linear scale release), `v2.0.1` (2026-07-19, review remediation), and `v2.0.2` (2026-07-22, standards migration and simplification refactors — latest). **Repo security posture:** Dependabot vulnerability alerts + automated security fixes enabled (post-v1.0.0 hygiene pass, 2026-07-07).
