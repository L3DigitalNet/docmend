...
- **MS-2 calibration checkpoint (2026-07-06, RQ-022):** executed against the §17.2 three-axis boundary matrix (committed under `tests/fixtures/weird_documents/encoding_floor/`; full 60-cell tabulation in the session log). Outcome: **keep 20** — lowering the floor from 20 to 16/12/8 raised confident false-accepts at every step (19 → 33 → 39 → 46) while recovering at most 8 false-skips, so no value below 20 satisfies the "admits no new false-accepts" rule; the same direction holds for the typical-prose (spread-placement) slice alone (false-accepts 2 → 4 → 5 → 6). Detection confidence was bimodal — 0 of 60 verdicts landed in 0.00–0.80, clustering at ~0.9/~1.0 — confirming the floor, not the confidence threshold, is the operative gate for short files. The lone residual is the sole-detector R-001 case: a confident Western-family misdetection whose decode differs from cp1252 (e.g. `naïve` → `naļve`), floor-independent and pinned as `western-cp1252-inequivalent.txt`. The checkpoint is closed; the family-aware table and ratio signal remain deferred behind the OQ-020 seam.
